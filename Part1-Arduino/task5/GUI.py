...
class PIDControllerApp:
    ...
    def create_widgets(self):
        ...
        self.setpoint_scale = tk.Scale(self.root, from_=0, to=1023, orient=tk.HORIZONTAL)
        self.setpoint_scale.grid(row=4, column=1)
    ...
    def update_setpoint(self):
        if self.serial_port and self.serial_port.is_open:
            value = self.setpoint_scale.get()
            self.serial_port.write(f"s={value}\n".encode())
    ...
    def serial_thread(self):
        while self.running:
            ...
            if line.startswith("Current Position:"):
                ...
                self.current_position_value.config(text=current_position)
            elif line.startswith("Set Point:"):
                set_point = line.split(":")[1].strip()
                self.setpoint_display_value.config(text=set_point)
            time.sleep(0.1)
